		...
		: ThreadedKernel(argc, argv)
{
	...
	for (int i = 1; i < argc; i++) {
		if (strcmp(argv[i], "-s") == 0) 
		{
			...
			//cout<<"-prio:"<<execPriority[execfileNum]<<endl;
		}	
		else if (strcmp(argv[i], "-burst") == 0) {
			execBurstTime[execfileNum] = atoi(argv[++i]);	
			//cout<<"-burst:"<<execPriority[execfileNum]<<endl;
		}
		/*else if (strcmp(argv[i], "PRIORITY") == 0) {
			kernel->scheduler->setSchedulerType(Priority);
		}
		else if (strcmp(argv[i], "SJF") == 0) {
			kernel->scheduler->setSchedulerType(SJF);
		}
		else if (strcmp(argv[i], "FCFS") == 0) {
			kernel->scheduler->setSchedulerType(FIFO);
		}*/
	}
...
UserProgKernel::Run()
{
	...
	for (int n=1;n<=execfileNum;n++)
		...
		//cout<<"userkernel.h run"<<endl;	
		if(kernel->scheduler->getSchedulerType() == Priority )
		{	
//			cout<<"userkernel priority"<<endl;
			t[n]->setPriority(execPriority[n]);
//			cout<<t[n]->getPriority();
		}
		
		else if(kernel->scheduler->getSchedulerType() == SJF)
		{
//			cout<<"userkernel SJF"<<endl;
			t[n]->setBurstTime(execBurstTime[n]);
//			cout<<t[n]->getBurstTime();
		}
		else if(kernel->scheduler->getSchedulerType() == FIFO)
		{
			cout<<"userkernel FIFO"<<endl;
		}

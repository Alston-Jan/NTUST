...
void Alarm::CallBack()
{
	...
	{ // there's someone to preempt
			...
			kernel->scheduler->getSchedulerType() == Priority)
		{
			...
			std::cout<<"YieldOnReturn"<<std::endl;
			interrupt->YieldOnReturn();
		}
	}
//	std::cout<<kernel->scheduler->getSchedulerType()<<std::endl;
}
...
void Alarm::Log(char c)
{
	...
	std::ofstream file;
	file.open("NachOS.log", ios::app);
	if (c == 'o' || c == 'O')
	{
		file << "[B10915045_Log]error" << endl;
		cout << "[B10915045_Log]error" << endl;
	}
	else
	{
		file << "[B10915045_Log]" << c << endl;
		cout << "[B10915045_Log]" << c << endl;
	}
	kernel->interrupt->SetLevel(oldLevel);
	file.close();
}

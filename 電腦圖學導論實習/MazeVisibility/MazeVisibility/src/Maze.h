...
class WallData {
public:
    WallData(const float* _start, const float* _end, const float* _color, const float& _depth) {
        for (size_t cp = 0; cp < 2; cp++) {
            this->start[cp] = _start[cp];
            this->end[cp] = _end[cp];
        }
        for (size_t cp = 0; cp < 3; cp++) {
            this->color[cp] = _color[cp];
        }
        this->depth = _depth;
    }
...
public:
    ...
    Maze(const int num_x, const int num_y, const float size_x, const float size_y);
...
public:
    ...
    void Draw_Wall(LineSeg wall, LineSeg L, LineSeg R, LineSeg front);
    ...
    void findAllDepth(Cell* cell, std::vector<WallData>& visibleWall, std::vector<Cell*>& vistiedCells);

    void drawWellWithCliping(WallData& wall);
...
public:
    ...
    static constexpr float Far = 200.0f;
};

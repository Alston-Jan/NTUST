...
static void redrawCB(Fl_Widget*, void* v){ auto* win = static_cast<TrainWindow*>(v); if (win && win->trainView) win->trainView->damage(1); }
...
//========================================================================
{
	...
	begin();	// add to this widget
	{
		...
		trainView = new TrainView(5,5,590,590,0);
		trainView->tw = this;
		...
		widgets->begin();
		// --- Lighting toggles ---
		lightDir   = new Fl_Check_Button(605, pty, 170, 20, "Directional (L0)"); lightDir->value(1); lightDir->callback(redrawCB, this); pty += 22;
		lightPoint = new Fl_Check_Button(605, pty, 170, 20, "Point (L1)");       lightPoint->value(1); lightPoint->callback(redrawCB, this); pty += 22;
		lightSpot  = new Fl_Check_Button(605, pty, 170, 20, "Spot (L2)");         lightSpot->value(1); lightSpot->callback(redrawCB, this); pty += 22;
		...
		Fl_Button* btnNone = new Fl_Button(10, 300, 120, 25, "No Effect");
		btnNone->callback([](Fl_Widget*, void* userdata) {
			((TrainWindow*)userdata)->postEffectMode = 0;
			});
		btnNone->user_data(this);

		Fl_Button* btnPixel = new Fl_Button(10, 330, 120, 25, "Pixelization");
		btnPixel->callback([](Fl_Widget*, void* userdata) {
			((TrainWindow*)userdata)->postEffectMode = 1;
			});
		btnPixel->user_data(this);

		Fl_Button* btnToon = new Fl_Button(10, 360, 120, 25, "Toon Shading");
		btnToon->callback([](Fl_Widget*, void* userdata) {
			((TrainWindow*)userdata)->postEffectMode = 2;
			});
		btnToon->user_data(this);
		...
		// --- Post-process Effect (Pixel / Toon) ---
		pty += 10;
		Fl_Box* effLabel = new Fl_Box(605, pty, 120, 20, "Post Effect");
		effLabel->align(FL_ALIGN_LEFT | FL_ALIGN_INSIDE);
		pty += 20;

		Fl_Group* effGroup = new Fl_Group(600, pty, 195, 70);
		effGroup->begin();
		effectNone = new Fl_Round_Button(605, pty, 150, 20, "None");
		effectNone->type(FL_RADIO_BUTTON);
		effectNone->value(1);                          // 預設：不做特效
		effectNone->selection_color((Fl_Color)3);
		effectNone->callback((Fl_Callback*)damageCB, this);

		pty += 22;
		effectPixel = new Fl_Round_Button(605, pty, 150, 20, "Pixelization");
		effectPixel->type(FL_RADIO_BUTTON);
		effectPixel->value(0);
		effectPixel->selection_color((Fl_Color)3);
		effectPixel->callback((Fl_Callback*)damageCB, this);

		pty += 22;
		effectToon = new Fl_Round_Button(605, pty, 150, 20, "Toon Shading");
		effectToon->type(FL_RADIO_BUTTON);
		effectToon->value(0);
		effectToon->selection_color((Fl_Color)3);
		effectToon->callback((Fl_Callback*)damageCB, this);
...
//========================================================================
{
	// Arc-length true distance advance or parameter advance
	float nct = static_cast<float>(m_Track.points.size());
	if (nct <= 0.f) nct = 1.f;

	if (arcLength->value()) {
		// desired physical distance per tick (tunable scale)
		float dsWorld = dir * (float)speed->value(); // you can scale by 0.1f if too fast
		m_Track.trainU = arclenAdvance(m_Track.trainU, dsWorld, trainView);
	} else {
		float vel = dir * (float)speed->value() * 0.1f;
		m_Track.trainU += vel;
	}

	while (m_Track.trainU >= nct) m_Track.trainU -= nct;
	while (m_Track.trainU < 0.f)  m_Track.trainU += nct;
}